...
function update() {
    ...
    for (let i = 0; i < plataformaArray.length; i++) {
        ...
        if (detectCollision(tokoala, plataforma) && velocidadY >= 0) {
        ...
        if (plataforma.type === "broken") {
            ...
            context.fillRect(plataforma.x, plataforma.y,
    ...
    while (plataformaArray.length > 0 && plataformaArray[0].y >= boardHeight) {
        plataformaArray.shift();
        newPlataforma();
        score += 10;
    }
...
function resetGame() {
    tokoala = {
        img: tokoalaDerechaImg, x: tokoalaX, y: tokoalaY,
...
function placePlataformas() {
    ...
    for (let i = 0; i < 8; i++) {
        newPlataforma(boardHeight - 60 * i - 120);
    }
}
